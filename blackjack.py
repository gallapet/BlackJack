...
class BlackJackGame:
    ...
    def play(self):
        ...
        if self._player.ace_value() == 21:  # Auto stand on being dealt 21
            ...
            if self._dealer.ace_value() != 21:  # Check if dealer also has 21, if not then blackjack!
                self._player._blackjack = True
                self.dealer_finished = True
        if self._dealer.ace_value() == 21 and self._player.ace_value() < 21:  
        ...
        while not self.player_finished:
            ...
            if player_choice.startswith("s"):
                self.player_finished = True
            elif player_choice.startswith("h") and self._player.ace_value() < 21:
                ...
                if self._player.ace_value() > 21:  # Check if player is bust
                    ...
                    self.player_finished = True
                    self.dealer_finished = True
                elif self._player.ace_value() == 21:  # Auto stand on 21
                    self.player_finished = True
        while self._dealer.ace_value() < 17 and not self.dealer_finished:  # Dealer draws to 16 and stands on 17
...
class Person:
    ...
    def ace_value(self):
        ...
        while final_value > 21 and self._hand._ace_count > 0:
            final_value -= 10
            self._hand._ace_count -= 1
